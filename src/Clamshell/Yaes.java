...
public class Yaes {
    ...
    {   
        // first, we convert inputs to hex 
        // then ensure correct sizes before continuing
        key = ""+bytesToHex(key.getBytes());
        IV = ""+bytesToHex(IV.getBytes());
        key = padOrTruncate(key, IV)[0];
        IV = padOrTruncate(key, IV)[1];
        
        plain = ""+bytesToHex(plain.getBytes());
            ...
            for (int i = 0; i < 4; i++) { // parse init vector into 4x4 matrix
                ...
                {
                    if (line.length() < 32) {
                        line = String.format("%032x",Integer.parseInt(line, 16)); // ***** HOW DOES THIS WORK?!?!?!??!
                    }
                    state = new int[4][4];
                ...
                {
                    System.exit(-5); // throw -5 code for somehow things not hex (ensure all hexadecimal!!!)
                }
