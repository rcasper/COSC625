...
/**
 * ZAES - AES in 256 CBC mode
 * For COSC 625 Fall 2013
 * @author Andrew Ramsey
 */
public class Zaes {
    
    public int[][] mc2 ={{0x00, 0x02, 0x04, 0x06, 0x08, 0x0a, 0x0c, 0x0e, 0x10, 0x12, 0x14, 0x16, 0x18, 0x1a, 0x1c, 0x1e},
    ...
    public int[][] mc3 ={   {0x00,0x03,0x06,0x05,0x0c,0x0f,0x0a,0x09,0x18,0x1b,0x1e,0x1d,0x14,0x17,0x12,0x11},
    ...
    public int[][] mc9 = {   {0x00,0x09,0x12,0x1b,0x24,0x2d,0x36,0x3f,0x48,0x41,0x5a,0x53,0x6c,0x65,0x7e,0x77},
    ...
    public int[][] mc11 = {  {0x00,0x0b,0x16,0x1d,0x2c,0x27,0x3a,0x31,0x58,0x53,0x4e,0x45,0x74,0x7f,0x62,0x69},
    ...
    public int[][] mc13 = {  {0x00,0x0d,0x1a,0x17,0x34,0x39,0x2e,0x23,0x68,0x65,0x72,0x7f,0x5c,0x51,0x46,0x4b},
    ...
    public int[][] mc14 = {  {0x00,0x0e,0x1c,0x12,0x38,0x36,0x24,0x2a,0x70,0x7e,0x6c,0x62,0x48,0x46,0x54,0x5a},
    ...
    public ArrayList decryptFile(File f) throws Exception
    {
        try{
            input = new BufferedReader(new FileReader(f));
            String hex32 = input.readLine();
            String hex64 = input.readLine();
    ...
    public void encryptFile(String userName, ArrayList table) throws Exception
    {
            FileWriter out;
            
            try
            {
                out = new FileWriter(userName + ".enc");
            }
            catch (Exception e)
            {
                return;
            }
            String lines[] = new String[table.size()];
            
            for (int p = 0; p < table.size(); ++p)
            {
                lines[p] =  ""+table.get(p); // fill lines with ArrayList Strings
            }
     ...
     * @return 
     */
    public boolean validatePassword(String password, String keyIV)
    {
            String passwordHex = padOrTruncate(String.copyValueOf(bytesToHex(password.getBytes())));
            System.out.println("key + IV:\n" + keyIV);
            System.out.println("password entered:\n" + passwordHex);
            // now validate if password is correct. if so, continue the decryption
            if (keyIV.matches(passwordHex))
            {
                JOptionPane.showMessageDialog(null, "Validated", "Welcome", JOptionPane.PLAIN_MESSAGE);
                isValidated = true;
                key = keyIV.substring(0, 64);
                iv = keyIV.substring(64, 96);
            }
        return true;
    }    
     ...
     * @return 
     */
    private String padOrTruncate(String pw)
    {
        String ret = pw;
        ...
        if (pw.length() < 96)
        {
            while (ret.length() < 96)
            {
            ret += 0; // pad with zeros
                if (ret.length() == 96) // just to make sure
                {
                    return ret;
                }
            }
        ...
        else if (pw.length() > 96)
        {
            ret = pw.substring(0, 96); // truncate
        }
        return ret;
    }
    
    // convert GUI Strings to Hex for AES
    private char[] bytesToHex(byte[] raw) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      hex[i * 2 + 0] = hexDigit[highIndex];
      hex[i * 2 + 1] = hexDigit[lowIndex];
    }
    return hex;
  }
    ...
    // convert back into screen input
